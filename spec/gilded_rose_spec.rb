...
describe GildedRose do
  ...
  let(:conjured) { Item.new(name='Conjured', sell_in=5, quality=20 )}
  let(:items) { [elixir, brie, sulfuras, concert, conjured] }
  ...
  context 'Conjured' do
    it 'quality decreases by 2 with each day until it reaches 0' do
      expect { gilded_rose.update_quality }.to change { conjured.quality }.by -2
    end

    it 'quality decreases by 4 with each day once the sell by date has passed' do
      6.times { gilded_rose.update_quality }
      expect { gilded_rose.update_quality }.to change { conjured.quality }.by -4
    end
  end
end
